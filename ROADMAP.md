...
> **You are here:** Feature 16 (reacting to activity — **Consumer**, candidate A2) — **✅ merged to `main`
> 2026-09-25 (PR #33)** ([`specs/018-event-consumer`](specs/018-event-consumer/)). A Scala consumer
...
>
> The live walk corrected two documented claims, both now fixed: a brand-new user's **first** delivery
> already contains the item that created the list, so it is reported inside the `baseline` counts and there
> is **no `added` entry for it**; and the local broker substitute `eventing.support = "logging"` **prints
> nothing by default** — it logs at INFO under a `kalix.*` logger the dev-mode logback silences at `WARN`,
> so a walk that greps for published messages finds none though they were produced. One line in
> `include-dev-loggers.xml` restores it. **A diagnostic configured off by default reads exactly like a
...
>
> PR review added two things: the production action now bounds its own attempts through the same helper
> the retry test proves (`BoundedAttempts`), and one known limit is stated rather than left implicit —
> finished reminders are never evicted, so an in-process store grows until restart.
>
...
The roadmap is **open**: capabilities 1–16 are merged, and the three candidates this section used to
