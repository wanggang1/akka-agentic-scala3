...
- [x] T018 Live smoke test against Gemini (**authoritative proof of US1 recall + US2 isolation**). ✅ PASSED: `POST /chat/c-123 "my name is Ada"` → `"Nice to meet you, Ada!"`; then `POST /chat/c-123 "what is my name?"` → `"Your name is Ada!"` (recall across two requests on one session id); `POST /chat/c-999 "what is my name?"` → `"I don't know your name yet! You haven't told me."` (isolation); blank message → `400 message must not be blank`. Recorded in README's cap-4 "Verified live" note.
- [x] T019 Final `mvn verify` (offline suite green across all four capabilities) and confirm `.env` remains git-ignored before any commit. ✅ BUILD SUCCESS; `.env` git-ignored.
